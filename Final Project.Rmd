...
```{r}
df <- vacc %>%
  select(c(date, location, continent, total_cases, total_deaths, total_vaccinations, population, gdp_per_capita, extreme_poverty, human_development_index, life_expectancy )) %>%
   filter(date  == max(date)) %>%
   select(-date) %>%
   mutate(vaccination_received_rate = (total_vaccinations/population) *100)
...
```{r}
regfit.full = regsubsets(total_vaccinations ~ ., data = df.cont,  nvmax = 9, method="exhaustive") 
summary(regfit.full)
```
